---
title: 'Categorização de transações '
deprecated: false
hidden: true
metadata:
  robots: index
---
A categorização de transações de contas e de cartões de crédito são realizadas utilizando o modelo proprietário da Lina.

Para as empresas que possuem essa funcionalidade, a categoria da transação aparece como uma propriedade nas APIs de transações.

<Table align={["left","left"]}>
  <thead>
    <tr>
      <th>
        Categoria
      </th>

      <th>
        Subcategoria
      </th>
    </tr>
  </thead>

  <tbody>
    <tr>
      <td>
        Renda
      </td>

      <td>
        * Remuneração
        * Aposentadoria
        * Rendimentos e Resgates
        * Outras Rendas
        * Auxílios
      </td>
    </tr>

    <tr>
      <td>
        Transferências
      </td>

      <td>
        * Pagamento de Cartão
        * Pagamento de Boleto
        * Transferência entre Pessoas (P2P)
        * Transferência Mesma Titularidade
        * Review
        * Devolução/Estorno/Cashback
        * Saque
        * Depósito
      </td>
    </tr>

    <tr>
      <td>
        Taxas e Tarifas Bancárias
      </td>

      <td>
        * Tarifas de Conta
        * Juros e Multas
        * Anuidade Cartão
      </td>
    </tr>

    <tr>
      <td>
        Impostos e Governo
      </td>

      <td>
        * IOF e Impostos Financeiros
        * Impostos
        * Taxas e Serviços Públicos
        * IPTU
        * IPVA
      </td>
    </tr>

    <tr>
      <td>
        Investimentos
      </td>

      <td>
        * Renda Fixa ou Variável
        * Cripto
      </td>
    </tr>

    <tr>
      <td>
        Crédito
      </td>

      <td>
        * Empréstimos
        * Financiamento
        * Antecipação
        * Outros Créditos
      </td>
    </tr>

    <tr>
      <td>
        Alimentação e Bebidas
      </td>

      <td>
        * Restaurantes e Bares 
        * Delivery
        * Comércio
        * Mercado
        * Padaria
      </td>
    </tr>

    <tr>
      <td>
        Transporte e Viagens
      </td>

      <td>
        * Aplicativos e Táxi
        * Combustível e Estacionamento
        * Transporte Público
        * Manutenção Automotiva
        * Viagens e Hospedagem
      </td>
    </tr>

    <tr>
      <td>
        Compras
      </td>

      <td>
        * Vestuário
        * Eletrônicos
        * Lojas de Departamento
        * E-commerce e Marketplaces
        * Outras compras
      </td>
    </tr>

    <tr>
      <td>
        Serviços Digitais
      </td>

      <td>
        * Streaming
        * Jogos
        * Subscrições
        * Outros Serviços Digitais
      </td>
    </tr>

    <tr>
      <td>
        Educação
      </td>

      <td>
        * Mensalidades e Cursos

        * Material Educativo
      </td>
    </tr>

    <tr>
      <td>
        Saúde
      </td>

      <td>
        * Farmácia  
        * Médicos e Clínicas
        * Bem-estar e Estética
      </td>
    </tr>

    <tr>
      <td>
        Habitação e Infraestrutura
      </td>

      <td>
        * Utilidades
        * Aluguel/Condomínio
      </td>
    </tr>

    <tr>
      <td>
        Serviços Profissionais e Manutenção
      </td>

      <td>
        * Manutenção, Reparos, Construção
        * Logística e Entregas
        * Serviços Técnicos e Consultoria
      </td>
    </tr>

    <tr>
      <td>
        Receita e Despesa Operacional
      </td>

      <td>
        * Pagamento a Fornecedor           
        * Transferência para PF           
        * Receita
      </td>
    </tr>

    <tr>
      <td>
        Seguros
      </td>

      <td>
        * Seguros
      </td>
    </tr>

    <tr>
      <td>
        Entretenimento e Recreação
      </td>

      <td>
        * Entretenimento e Recreação
        * Casas de aposta
      </td>
    </tr>

    <tr>
      <td>
        Câmbio
      </td>

      <td>
        * Câmbio
      </td>
    </tr>

    <tr>
      <td>
        Pets
      </td>

      <td>
        * Pets
      </td>
    </tr>

    <tr>
      <td>
        Outros
      </td>

      <td>
        * Outros
      </td>
    </tr>
  </tbody>
</Table>

<br />